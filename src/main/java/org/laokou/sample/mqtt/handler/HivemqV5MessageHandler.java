...
@Slf4j
@Component
class HivemqV5MessageHandler implements MessageHandler {
	...
	public void handle(MqttMessage mqttMessage) {
		try {
			log.info("【Hivemq-V5】 => 接收到MQTT消息，topic: {}, message: {}", mqttMessage.getTopic(),
					new String(mqttMessage.getPayload(), StandardCharsets.UTF_8));
		}
		catch (Exception e) {
			log.error("【Hivemq-V5】 => MQTT消息处理失败，Topic：{}，错误信息：{}", mqttMessage.getTopic(), e.getMessage(), e);
		}
	}
